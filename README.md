...
| `make seed`          | Seed database using `DatabaseSeeder`        |
...
| `make telescope-clear`       | Flush all Telescope entries from the database                 |
...
OpenAPI docs are generated by [Dedoc Scramble](https://scramble.dedoc.co).

- UI (Swagger): `http://localhost:8080/docs/api`
- JSON spec: `http://localhost:8080/docs/api.json`

Useful commands:

- Analyze documentation issues:
  `make artisan cmd="scramble:analyze"`
- Export OpenAPI JSON:
  `make artisan cmd="scramble:export --path=storage/api-docs/openapi.json"`

To customize Scramble config, publish the package config first:

```bash
make artisan cmd="vendor:publish --provider='Dedoc\\Scramble\\ScrambleServiceProvider' --tag='scramble-config'"
```

By default, Scramble UI routes are available in `local` environment.
For non-local environments, configure docs authorization (for example, via `viewApiDocs` gate) before exposing docs.

## Dev Tools

| Tool | URL | Enabled by |
|------|-----|------------|
| **Scramble** (OpenAPI docs) | `http://localhost:8080/docs/api` | Always on in `local` env |
| **Telescope** (request/query/job inspector) | `http://localhost:8080/telescope` | `TELESCOPE_ENABLED=true` in `.env` |

> Telescope is a **dev-only** package (`require-dev`). It is automatically excluded when the production image runs `composer install --no-dev`. Set `TELESCOPE_ENABLED=false` (or omit it) to disable recording without removing the package.
...
| `GET`  | `/auth/me`       | ✔    | —                                                                            | `200` `{ "user": { "username", "email" } }` |
